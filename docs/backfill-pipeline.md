...
1. **`split-issue.sh`** -- validates the PDF, parses the table of contents, splits into individual article PDFs, normalizes author names, and generates OJS import XML. Does not touch OJS.
2. **`import.sh`** -- loads the generated XML into OJS via the Native Import/Export CLI.
...
`split-issue.sh` runs five steps in sequence. Each step is a standalone Python script.
...
- **PDF embedding:** Each article PDF is base64-encoded and embedded inline. A typical issue produces a 20--50 MB XML file. Use `--dry-run` to skip embedding for testing XML structure.
...
Classification is by title: "Editorial" maps to Editorial, "Book Reviews" maps to Book Review Editorial (the section intro). Individual book reviews detected within the Book Reviews pages get section `Book Reviews`. Everything else is `Articles`.
...
| `--only=<step>` | Run a single step only. Valid steps: `preflight`, `parse_toc`, `split`, `normalize`, `generate_xml`. |
...
| `--journal=<path>` | `journal` | OJS journal path (URL path component). |
...
- **TOC parser tuned for recent issues.** The `CONTENTS` page format, text extraction patterns, and page offset detection are calibrated against issues from the last ~10 years. Older issues with different layouts may need manual `toc.json` adjustment or parser changes.
...
python3 backfill/generate_xml.py backfill/output/EA-vol37-iss1/toc.json -o import.xml --dry-run
